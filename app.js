...
function openNewMotifModal(){const m=$("newMotifModal");if(!m)return;["newMotifCode","newMotifLabel"].forEach(id=>$(id).value="");$("newMotifPresence").value="1";$("newMotifAbsence").value="0";$("newMotifCapacity").checked=true;m.hidden=false;m.style.display="flex";setTimeout(()=>$("newMotifCode")?.focus(),0)}
function closeNewMotifModal(){const m=$("newMotifModal");if(!m)return;m.hidden=true;m.style.display="none"}
async function createNewMotif(){const code=String($("newMotifCode")?.value||"").trim().toUpperCase(),label=String($("newMotifLabel")?.value||"").trim();if(!code)return notify("Saisissez un code motif.");if(S.motifs.some(m=>String(m.Code||"").trim().toUpperCase()===code))return notify("Ce code motif existe déjà.");const fields={Code:code,Libelle:label||code,Presence_Equivalent:num($("newMotifPresence")?.value),Absence_Equivalent:num($("newMotifAbsence")?.value),Compte_Capacite:$("newMotifCapacity")?.checked===true,Actif:true};await grist.getTable(T.motifs).create({fields});closeNewMotifModal();await load();const created=S.motifs.find(m=>String(m.Code||"").trim().toUpperCase()===code);if(created){S.selectedMotif=created.id;renderMassMotifs()}notify(`Motif ${code} ajouté et disponible dans la feuille de présence et les rapports.`)}
