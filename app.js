...
function render(){const opts=activeTeam().sort((a,b)=>String(a.nom).localeCompare(String(b.nom))).map(x=>`<option value="${x.id}">${esc(x.nom)}</option>`).join("");$("person").innerHTML='<option value="">Toute l’équipe</option>'+opts;renderReportTeams();renderReconcileResources();chips();pilotage();renderRecent();alerts();renderForecast();setupAnnualYears();renderAnnualAlerts();renderMassFilters();renderMassMotifs();renderMotifInfo();renderMassCalendar();renderLockBadge()}
...
function periodAlerts(scope){
 const out=[],add=(p,l,v,dt="")=>{const s=severity(p,v);if(s)out.push({s,l,v,u:p.Unite||"",dt,code:p.Code_Alerte||""})},team=scope.people,rr=scope.rows;
 S.params.filter(p=>p.Actif!==false).forEach(raw=>{const p=raw.Code_Alerte==="PRES_PHY"?physicalParam():raw;
  if(p.Code_Alerte==="ABS_IND")team.forEach(pe=>{let v=0;rr.filter(r=>r.Ressource===pe.id).forEach(r=>v+=num(motif(r.Motif)?.Absence_Equivalent));add(p,`${pe.nom} · absences sur la période`,v)});
...
function generateHtmlReport(){
 ...
 const st=capacityStats(q.rows),alerts=periodAlerts(q),real=q.rows.filter(r=>isDateLocked(iso(r.Date))).length,open=q.rows.length-real;
 const byPerson=q.people.map(pe=>{const rr=q.rows.filter(r=>r.Ressource===pe.id),s=capacityStats(rr);return{name:pe.nom||("Ressource "+pe.id),presence:s.presence,absence:s.absence,remote:s.remote,formation:s.formation,work:s.work,rate:s.work?s.presence/s.work*100:0}}).sort((a,b)=>a.name.localeCompare(b.name,"fr"));
 const motifCounts={};q.rows.forEach(r=>{const m=motif(r.Motif);if(!m)return;motifCounts[m.Code]=(motifCounts[m.Code]||0)+1});
 const alertRows=alerts.length?alerts.map(a=>`<tr><td>${a.s==="red"?"🔴 Critique":"🟠 Vigilance"}</td><td>${esc(a.code)}</td><td>${esc(a.l)}</td><td>${a.dt?fmtReportDate(a.dt):"Période"}</td><td>${Number(a.v).toFixed(1)} ${esc(a.u||"")}</td></tr>`).join(""):'<tr><td colspan="5">Aucune alerte sur la période.</td></tr>';
 const peopleRows=byPerson.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.presence.toFixed(1)}</td><td>${x.absence.toFixed(1)}</td><td>${x.remote.toFixed(1)}</td><td>${x.formation.toFixed(1)}</td><td>${x.rate.toFixed(1)} %</td></tr>`).join("");
 const motifRows=Object.entries(motifCounts).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<tr><td>${esc(k)}</td><td>${v}</td></tr>`).join("");
 const html=`<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>Rapport RH - ${esc(q.label)}</title><style>body{font:14px Arial,sans-serif;color:#172033;margin:28px}h1{margin:0;color:#173b68}.meta{color:#667085;margin:6px 0 22px}.k{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}.k div{border:1px solid #dce3ea;border-radius:10px;padding:12px}.k b{display:block;font-size:22px;color:#173b68}.k span{font-size:11px;color:#667085}table{width:100%;border-collapse:collapse;margin:10px 0 22px}th,td{padding:8px;border-bottom:1px solid #e5e7eb;text-align:left}th{background:#f5f7fa}h2{margin-top:24px;font-size:17px}.actions{margin-bottom:18px}.actions button{padding:9px 14px;border:0;border-radius:8px;background:#173b68;color:white;font-weight:700}@media print{.actions{display:none}body{margin:12mm}.k{break-inside:avoid}table{break-inside:auto}tr{break-inside:avoid}}</style></head><body><div class="actions"><button onclick="window.print()">Imprimer / PDF</button></div><h1>Rapport RH — ${esc(q.label)}</h1><div class="meta">Période du ${fmtReportDate(q.from)} au ${fmtReportDate(q.to)} · ${q.people.length} ressource(s) · Généré le ${new Date().toLocaleString("fr-FR")}</div><div class="k"><div><b>${st.presence.toFixed(1)}</b><span>jours de présence équivalente</span></div><div><b>${st.absence.toFixed(1)}</b><span>jours d’absence équivalente</span></div><div><b>${st.remote.toFixed(1)}</b><span>jours de télétravail</span></div><div><b>${st.formation.toFixed(1)}</b><span>jours de formation</span></div><div><b>${st.capacity.toFixed(1)} %</b><span>taux de présence / capacité</span></div><div><b>${real}</b><span>saisies 🔒 réalisées</span></div><div><b>${open}</b><span>saisies ✏️ ouvertes</span></div><div><b>${alerts.length}</b><span>alertes sur la période</span></div></div><h2>Indicateurs par ressource</h2><table><thead><tr><th>Ressource</th><th>Présence (j)</th><th>Absence (j)</th><th>Télétravail (j)</th><th>Formation (j)</th><th>Taux</th></tr></thead><tbody>${peopleRows}</tbody></table><h2>Répartition des motifs</h2><table><thead><tr><th>Motif</th><th>Nombre de saisies</th></tr></thead><tbody>${motifRows||'<tr><td colspan="2">Aucune donnée.</td></tr>'}</tbody></table><h2>Alertes sur la période</h2><table><thead><tr><th>Niveau</th><th>Code</th><th>Alerte</th><th>Date</th><th>Valeur</th></tr></thead><tbody>${alertRows}</tbody></table></body></html>`;
...
function buildPortableReportHtml(){
 ...
 const alerts=periodAlerts(q).map(a=>({level:a.s,code:a.code||"",label:a.l||"",date:a.dt||"",value:Number(a.v)||0,unit:a.u||""}));
 const payload=JSON.stringify({title:"Rapport RH",scope:q.label,from:q.from,to:q.to,people,rows,alerts}).replace(/</g,"\\u003c");
 ...
 function render(){const ids=selected(),single=Number($('single').value||0);if(single){ids.clear();ids.add(single);document.querySelectorAll('.pick').forEach(x=>x.checked=ids.has(Number(x.value)))}const ppl=D.people.filter(p=>ids.has(p.id)),rows=D.rows.filter(r=>ids.has(r.person)),s=stat(rows);$('subtitle').textContent=D.scope+' · du '+fd(D.from)+' au '+fd(D.to)+' · '+ppl.length+' ressource(s)';$('cards').innerHTML=[['Présence',fmt(s.presence)+' j'],['Absence',fmt(s.absence)+' j'],['Télétravail',fmt(s.remote)+' j'],['Formation',fmt(s.formation)+' j'],['Taux présence',fmt(s.rate)+' %'],['Réalisé 🔒',s.real],['Ouvert ✏️',s.open],['Alertes',D.alerts.length]].map(x=>'<div class="card"><b>'+x[1]+'</b><span>'+x[0]+'</span></div>').join('');
 const per=ppl.map(p=>{const z=stat(rows.filter(r=>r.person===p.id));return {p,z}});$('peopleRows').innerHTML=per.map(o=>'<tr><td><b>'+o.p.name+'</b></td><td>'+fmt(o.z.presence)+' j</td><td>'+fmt(o.z.absence)+' j</td><td>'+fmt(o.z.remote)+' j</td><td>'+fmt(o.z.formation)+' j</td><td><div>'+fmt(o.z.rate)+' %</div><div class="bar"><i style="width:'+Math.min(100,o.z.rate)+'%"></i></div></td></tr>').join('')||'<tr><td colspan="6" class="empty">Aucune ressource sélectionnée.</td></tr>';
 ...
 $('alerts').innerHTML=D.alerts.map(a=>'<tr><td class="'+(a.level==='red'?'crit':'warn')+'">'+(a.level==='red'?'● Critique':'● Vigilance')+'</td><td>'+a.code+'</td><td>'+a.label+'</td><td>'+fd(a.date)+'</td><td>'+fmt(a.value)+' '+a.unit+'</td></tr>').join('')||'<tr><td colspan="5" class="empty">Aucune alerte sur la période.</td></tr>';
