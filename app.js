...
function csvParse(t){t=t.replace(/^\\uFEFF/,"").replace(/\\r/g,"");let l=t.split("\\n").filter(x=>x.trim());let sep=(l[0].match(/;/g)||[]).length>=(l[0].match(/,/g)||[]).length?";":",";let hh=csvLine(l[0],sep).map(x=>x.trim());return{headers:hh,rows:l.slice(1).map((x,n)=>{let v=csvLine(x,sep),o={_line:n+2};hh.forEach((k,i)=>o[k]=(v[i]||"").trim());return o})}}
...
function csvAnalyze(p){let miss=CSV_HEADER.filter(x=>!p.headers.includes(x));if(miss.length)throw Error("Colonnes manquantes : "+miss.join(", "));let nr=new Map(),nt=new Map(),rows=[];for(let r of p.rows){let d=[],res=csvResource(r),tm=csvTeam(r.Equipe_Code),mo=csvMotif(r.Motif),bad=false,st=r.Statut||"Prévisionnel";if(!r.Nom_Ressource&&!r.Email){d.push("Ressource obligatoire");bad=true}if(!/^\d{4}-\d{2}-\d{2}$/.test(r.Date)){d.push("Date AAAA-MM-JJ");bad=true}if(!mo){d.push("Motif inconnu");bad=true}if(!["Prévisionnel","Confirmé","Réalisé"].includes(st)){d.push("Statut invalide");bad=true}let key=(r.Email||r.Nom_Ressource).toLowerCase(),teamKey=(r.Equipe_Code||"").trim().toLowerCase();if(r.Equipe_Code&&!tm&&!nt.has(teamKey))nt.set(teamKey,{key:teamKey,Code:r.Equipe_Code.trim(),Libelle:r.Equipe_Code.trim(),Description:"Créée automatiquement par import CSV"});if(!res&&!nr.has(key))nr.set(key,{...r,key,teamId:tm?.id||0,teamKey});if(r.Equipe_Code&&!tm)d.push("Équipe à créer automatiquement");let old=res?S.presence.find(x=>x.Ressource===res.id&&iso(x.Date)===r.Date):null;rows.push({...r,status:st,res,mo,key,teamKey,old,bad,diag:d.join(" · ")||"OK",action:res?(old?"Modifier":"Créer"):"Créer ressource + présence"})}return{rows,newResources:nr,newTeams:nt}}
