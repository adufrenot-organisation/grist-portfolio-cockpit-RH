...
function generateHtmlReport(){
 ...
 const st=capacityStats(q.rows),alerts=($("reportIncludeAlerts")?.checked!==false?periodAlerts(q):[]),real=q.rows.filter(r=>isDateLocked(iso(r.Date))).length,open=q.rows.length-real;
...
function buildPortableReportHtml(){
 ...
 const includeAlerts=$("reportIncludeAlerts")?.checked!==false;
 const includeChart=$("reportIncludeChart")?.checked!==false;
 ...
 const payload=JSON.stringify({title:"Rapport RH",scope:q.label,from:q.from,to:q.to,people,rows,alerts,includeAlerts,includeChart}).replace(/</g,"\\u003c");
 ...
 function render(){const ids=selected(),single=Number($('single').value||0);if(single){ids.clear();ids.add(single);document.querySelectorAll('.pick').forEach(x=>x.checked=ids.has(Number(x.value)))}const ppl=D.people.filter(p=>ids.has(p.id)),rows=D.rows.filter(r=>ids.has(r.person)),s=stat(rows);$('subtitle').textContent=D.scope+' · du '+fd(D.from)+' au '+fd(D.to)+' · '+ppl.length+' ressource(s)';$('cards').innerHTML=[['Présence',fmt(s.presence)+' j'],['Absence',fmt(s.absence)+' j'],['Télétravail',fmt(s.remote)+' j'],['Formation',fmt(s.formation)+' j'],['Taux présence',fmt(s.rate)+' %'],['Réalisé 🔒',s.real],['Ouvert ✏️',s.open],['Alertes',D.alerts.filter(a=>!a.personId||ids.has(a.personId)).length]].map(x=>'<div class="card"><b>'+x[1]+'</b><span>'+x[0]+'</span></div>').join('');
